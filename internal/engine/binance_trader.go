...
// BinanceTrader orchestrates the trading bot's operations.
...
// NewBinanceTrader creates a new BinanceTrader instance.
func NewBinanceTrader(cfg *config.Config, pnlLogger *logging.PnlLogger, marketPulseLogger *logging.MarketPulseLogger, notifier notifier.Notifier, repo database.Repository, startTime time.Time, sessionNumber int, availableBalance decimal.Decimal, apiKey, apiSecret string) (*BinanceTrader, error) {
	client, err := exchange.New(cfg, apiKey, apiSecret)
	if err != nil {
...
func (e *BinanceTrader) GetStatus() Status {
	return Status{
		IsRunning: e.IsRunning(),
		Uptime:    e.GetUptime().String(),
		Balance:   e.availableBalance.InexactFloat64(),
	}
}

func (e *BinanceTrader) GetUptime() time.Duration {
...
func (e *BinanceTrader) Run(ctx context.Context) {
	...
	for {
		...
		case <-ticker.C:
			log.Print("Reconciliation loop triggered")
...
func (e *BinanceTrader) closePosition(ctx context.Context, side futures.SideType, quantity decimal.Decimal, exitReason string) {
	...
	if pnl.Abs().GreaterThan(decimal.NewFromFloat(e.config.SessionBudget)) {
		log.Printf("Logic Error: PNL exceeds session budget, ignoring. PNL: %s", pnl.StringFixed(2))
...
func (e *BinanceTrader) updateDashboard(ctx context.Context) {
	...
	if err != nil {
		...
		if err != nil {
			log.Printf("Failed to parse available balance in dashboard: %v", err)
		} else {
...
func (e *BinanceTrader) hardShutdown(status string) {
	log.Printf("CRITICAL: Margin Depleted or Panic Triggered. Ending Session.")
	go e.notifier.Notify(fmt.Sprintf("CRITICAL: Shutting down. Status: %s", status))
